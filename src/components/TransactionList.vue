...
<template>
    <header class="d-flex justify-content-between align-items-center">
        <h5>
            <Icon name="tx" class="me-2" :spin-watch="transactions" />
            <span v-if="pending" class="ml-2">Pending</span> Transactions
        </h5>
        <router-link class="small link-secondary text-decoration-none" :to="{ name: 'transactions' }">View All>>
        </router-link>
    </header>
    ...
    <ListGroup flush class="overflow-auto" style="height: 387px">
        <ListGroupItem v-for="(tx, i) in transactions" :key="i" hover>
            ...
            <div class="col-6 small text-truncate">
                ...
                <router-link class="link-dark" :to="{ name: 'wallet', params: { address: tx.from } }">{{ tx.from }}
                </router-link>
            </div>
            ...
            <div class="col-6 small text-truncate">
                ...
                <router-link class="link-dark" :to="{ name: 'wallet', params: { address: tx.to } }">{{ tx.to }}
                </router-link>
            </div>
        </ListGroupItem>
    </ListGroup>
